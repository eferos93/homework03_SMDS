...
Let's consider $\beta$ as the paramether of interest and $\gamma$ as the nuisance parameter. We can define the profile log-likelihood as follow:
$$ l_P(\beta)= \max_{\beta} l(\gamma, \beta; y) = l(\hat\gamma_{\beta}, \beta; y)$$
where $\hat\gamma_{\beta}$ is constrained MLE for $\gamma$ ì, with $\beta$.
...
The first plot represent the posterior uncertainty interval, or credibility interval, for $\theta$. We can see that the value of the posterior mean, represented by the light blue point, is around $2.6$. The thick blue segment shows 50% interval, that goes from  $2.3$ to $3.8$, whereas the thinner light blue line show the 90% interval.
...
Finally the third plot represent the marginal posterior denisty for the parameter $\theta$. Even if it is shown by tue posterior areas plot too, in this case it is represented through an hystogram. Changing the value of the parameter `binwidth` we can vary the number of bins of the histogram and so how many observations are summarized within each bin.
...
The second plot confirm our observation. The shaded area, which represent the 80% interval is wider then the one represented in the Laboratory.
...
First of all we can see that the posterior mean is around $0.35$. The 90% posterior interval is a bit asymmetrical, being a bit larger on the right side. This means that the distribution has a longer tail on the right side. The second and the tird plots confirm this observation. Finally, form the last plot we can see the denisty posterior distribution estimated form the 4 Markov chains that are quite similar.
