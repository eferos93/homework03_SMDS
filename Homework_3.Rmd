...
The first plot represent the posterior uncertainty interval, or credibility interval, for $\theta$. We can see that the value of the posterior mean, represented by the light blue point, is around $2.6$. The thick blue segment shows 50% interval, whereas the thinner light blue line show the 90% interval. So we can say that, with 50% probability the parameter $\theta$ falls between the values $2.3$ and $3.8$

The second plot represent the posterior areas. The thick blue line shows the value of the posterior mean. The curve represent the estimated posterior density. The light blue shaded area under the posterior density curve is the 80%  posterior uncertainty intervals. This plot shows all the information represented in the first one and some more, such as the posterior density.

Finally the third plot represent the marginal posterior denisty for the parameter $\theta$. Even if it is shown by tue posterior areas plot too, in this case it is represented through an hystogram. Changing the value of the parameter `binwidth` we can vary the number of bins of the histogram and so how many observations are summarized by each bin.
...
1. The beta distribution granantees this condition but it also limits the upper bound to 1. So the parameter $\lambda$, that is also the mean of the Possion distribution would be limited between 0 and 1 and this is not acceptable.

2. The normal distribution does not guarantee the positivity of the parameter so it's not acceptables.

3. The gamma distribution is defined in the interval $[0, + \infty]$. It guarantees that $\lambda > 0$ and it does not limit the upper boud. This one seems to be the best choice.
...
So we can see that the Gamma distribution is the coniugate prior of the Poisson distribution since the posterior is still a Gamma with updated parameters.
...
The value of $\sigma^2$ during the simulation varys a bit more that the simulation done during the Laboratory. Indeed, thanks to the `traceplot`, we can see that the majority of values are in a wider range, that goes from 1 to 3. Nevertheless the maximum reached value is lower the in the previous case.

The second plot confirm our observation. Indeed the shaded area, which represent the 80% interval is wider then the one represented in the Laboratory.
...
```
After having performed the MCMC simulation and having extracted the posterior distribution we produced some plots. From that we can obtain several information about the posterior distribution of the parameter of interest $p$. 

First of all we can see that the posterior mean is around $0.035$ and both the 50% posterior interval and the 90% posterior interval al not symmetrical. Indeed the first one is larger on the left side whereas the second one in wider on the right side. This means that the mass of the distribution is shifted through the left but it has a longer tail on the right side. The second and the tird plots confirm this observation. We can see that the posterior distribution is not symmetrical, its mass is shifted through 0 but it has a longer tail on the right side. Finally, form the last plot we can see the denisty posterior distribution estimated form the 4 Markov chains. They are quite similar but with some differences expecially in the more frequent values.
...
So we know that the Beta distribution is the coniugate prior of the Bernoulli distribution since the posterior is still a Beta with updated parameters.
